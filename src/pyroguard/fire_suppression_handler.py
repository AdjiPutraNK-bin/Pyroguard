...
class FireSuppressionHandler(Node):
    def __init__(self):
        ...
        self.fire_proximity_history = deque(maxlen=10)  # Track fire distances for retry
    ...
    def fire_position_callback(self, msg: PointStamped):
        try:
            ...
            if self.odom is not None:
                ...
                self.fire_proximity_history.append((dist, self.get_clock().now().nanoseconds / 1e9))
                self.get_logger().info(f"Updated current fire position: ({msg.point.x:.2f}, {msg.point.y:.2f}), distance: {dist:.2f}m", throttle_duration_sec=2.0)
        except Exception as e:
            self.get_logger().error(f"Fire position callback failed: {str(e)}")
            self.current_fire_position = None
    ...
    def select_and_suppress_fire(self):
        ...
        if dist <= self.suppression_distance:
            self.get_logger().info(f"Suppressing fire at dist {dist:.2f}m", throttle_duration_sec=2.0)
            ...
            self.current_fire_position = None
            self.fire_proximity_history.clear()
            ...
            self.get_logger().info(f"Fire too far to suppress: {dist:.2f}m > {self.suppression_distance}m", throttle_duration_sec=2.0)
            # Retry logic: Check if fire was recently close
            now = self.get_clock().now().nanoseconds / 1e9
            recent_close = any(d < self.suppression_distance * 1.5 and now - t < self.retry_timeout for d, t in self.fire_proximity_history)
            if recent_close:
                self.get_logger().info("Fire was recently close, keeping in approach mode for retry")
            else:
                self.current_fire_position = None
                self.fire_proximity_history.clear()
